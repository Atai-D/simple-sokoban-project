...
void player::move(int dx, int dy) {
    ...
    if (!game_level.is_cell_inside_level(next_row, next_column)) {
        return;
    }

    char &cell = game_level.get_level_cell(static_cast<size_t>(next_row), static_cast<size_t>(next_column));
    if (cell == FLOOR || cell == GOAL) {
        ...
        if (!game_level.is_cell_inside_level(target_row, target_column)) {
            return;
        }
        char &target_cell = game_level.get_level_cell(static_cast<size_t>(target_row), static_cast<size_t>(target_column));
        if (target_cell == FLOOR || target_cell == GOAL) {
            ...
            for (size_t row = 0; level_solved && row < game_level.get_rows(); ++row) {
                for (size_t column = 0; level_solved && column < game_level.get_columns(); ++column) {
                    char cell_to_test = game_level.get_level_cell(row, column);
